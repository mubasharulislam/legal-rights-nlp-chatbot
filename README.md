...
A Flask-based legal information retrieval chatbot for workplace harassment awareness in Pakistan. The project is designed as a Computational Linguistics / NLP portfolio system: it maps natural-language user questions to a curated legal knowledge base using TF-IDF retrieval, keyword scoring, and controlled response formatting.
...
- Keyword scoring for domain-specific legal and workplace vocabulary
...
The system treats each legal topic as a retrievable document. A topic document is constructed from its title, keywords, example questions, answer text, and procedural steps. User queries are normalized, vectorized with TF-IDF when available, and ranked against topic documents using cosine similarity plus a manually curated keyword score.

This makes the project a small-domain information retrieval system rather than an open-domain generative chatbot. The core NLP concepts include document representation, sparse vector retrieval, term weighting, n-gram features, lexical overlap, hybrid symbolic-statistical scoring, and threshold-based fallback behavior.
